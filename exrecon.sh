...
for i in {1..30}; do
nc -z 127.0.0.1 9051 2>/dev/null && break || true
sleep 1
done
fi
...
# decoy_flag is intentionally unquoted at call sites for word splitting.

# An empty string is safe with set -u since the variable is always defined.
...
scan_type=”${scan_type// /}”  # trim any accidental whitespace
rotate_tor_circuit
case “$scan_type” in
1)
...
echo
echo “— Nmap Findings —”
grep -hE “open|PORT|Service detection performed”   
“$output_dir”/scan_”$timestamp”.* 2>/dev/null || echo “  No findings captured.”
echo
if [[ -f “$output_dir/scan_$timestamp.nikto” ]]; then
echo “— Nikto Findings —”
grep -E “+|OSVDB|CVE” “$output_dir/scan_$timestamp.nikto” 2>/dev/null   
| sed ‘s/^/  /’ || echo “  No Nikto findings.”
echo
fi
echo “— Timeline —”
